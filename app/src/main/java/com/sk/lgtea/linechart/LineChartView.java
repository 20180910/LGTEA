package com.sk.lgtea.linechart;
...
public class LineChartView extends FramBase implements GestureDetector.OnGestureListener {
    ...
    private int defaultDataLineColor = Color.WHITE;
...
//        mBorderLinePaint.setAntiAlias(true);
        ...
        mPointPaint.setStrokeWidth(dp2px(1));
    ...
    private void drawDataLines(Canvas canvas) {
        ...
        for (int i = 0; i < showNum; i++) {
            ...
            mPointPaint.setColor(this.getResources().getColor(R.color.blue_qian));
            mPointPaint.setStyle(Paint.Style.STROKE);
            canvas.drawCircle(pointF.x, pointF.y, dp2px(2), mPointPaint);
            ...
            if (i > 0 && getPoint(i - 1).y <= pointF.y) {
                canvas.drawText(d, pointF.x - mTextPaint.measureText(d) / 2, pointF.y - dp2px(12), mTextPaint);
            } else {
                canvas.drawText(d, pointF.x - mTextPaint.measureText(d) / 2, pointF.y - dp2px(12), mTextPaint);
            }
